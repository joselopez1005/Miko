...
fun CompletionsDto.toChatMessageEntity(chatId: Int): ChatMessageEntity {
    choices.first().apply {
        return ChatMessageEntity(role = message.role, content = message.content, time = created, chatId = chatId)
    }
}

// Used to convert user message to ChatMessageEntity used to store response into DB
fun Message.toChatMessageEntity(chatId: Int): ChatMessageEntity {
    return ChatMessageEntity(role = role, content = content, time = time.toEpochSecond(ZoneOffset.systemDefault().rules.getOffset(time)), chatId)
}
